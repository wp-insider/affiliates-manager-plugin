...
<div id="confirm-continue-dialog" style="display: none" title="<?php _e( 'Submit to PayPal?', 'affiliates-manager' ) ?>">
	...
	<p><strong><?php echo sprintf( __( 'Your PayPal account will be charged %s!', 'affiliates-manager' ), esc_html(wpam_format_money( $this->viewData['total'], false )) ) ?></strong></p>
</div>
...
<div class="wrap">
	...
	<table class="widefat" style="width: 900px">
		...
		<?php foreach ($this->viewData['affiliates'] as $affiliate) { ?>
			<tr>
				...
				<td><?php echo esc_html(wpam_format_money($affiliate->balance))?></td>
				<td>-</td>
				 <td><?php echo esc_html(wpam_format_money($affiliate->paymentAmount, false))?></td>
				<td>=</td>
				<td><?php echo esc_html(wpam_format_money($affiliate->newBalance))?></td>
			</tr>
		<?php } ?>
		</tbody>
	</table>
	<table class="totalsTable" style="position: relative; width: 400px; left: 500px; border: 1px solid #aaa;">
	 ...
	 <td style="width: 150px;" class="moneyCell" id="subTotalCell"><?php echo esc_html(wpam_format_money($this->viewData['paymentTotal'], false))?></td>
		</tr>
		<tr>
			<td><?php echo sprintf( __( 'PayPal Fee<br /><small>2%% per payment, max %s1 per payment</small>', 'affiliates-manager' ), esc_html($currency) ) ?></td>
	 		<td class="moneyCell" id="paypalFeeCell"><?php echo esc_html(wpam_format_money($this->viewData['feeTotal'], false))?></td>
		</tr>
		<tr class="totalSeparatorRow"><td colspan="2"></td> </tr>
		<tr class="totalRow">
			<th><?php _e( 'Estimated Total', 'affiliates-manager' ) ?></th>
			<th class="moneyCell" id="totalCell"><?php echo esc_html(wpam_format_money($this->viewData['total'], false))?></th>
		</tr>
	</table>
